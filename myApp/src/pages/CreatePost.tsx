...
import Header from "../components/Header"; // Import Header component
import { makeRecipe } from "../logics/Recipe";
import { compressImage } from "../logics/Camera"; // Import compression logic

const CreatePost: React.FC = () => {
    const [title, setTitle] = useState<string>("");
    const [ingredients, setIngredients] = useState<string>("");
    const [instructions, setInstructions] = useState<string>("");
    const [photoBase64, setPhotoBase64] = useState<string | null>(null); // Store photo as base64
    const [error, setError] = useState<string | null>(null);
    const [successMessage, setSuccessMessage] = useState<string | null>(null);
    ...
    const handlePostUpload = async () => {
        ...
        try {
            const recipeData = {
                ...
                ingredients: ingredients.split("\n"),
                instructions: instructions.split("\n"),
                photoBase64: photoBase64 || undefined, // Convert null to undefined
            };
    
            const recipeId = await makeRecipe(recipeData); // Call updated `makeRecipe`
            setSuccessMessage(`Recipe "${title}" created successfully with ID: ${recipeId}`);
            setTitle("");
            setIngredients("");
            setInstructions("");
            setPhotoBase64(null);
        ...
        <IonPage>
            ...
            <IonContent fullscreen>
                <IonCard>
                    <IonCardContent>
                        <UploadPhotoButton
                            onPhotoUpload={async (compressedImage) => {
                                try {
                                    setPhotoBase64(compressedImage); // Save compressed image as base64
                                } catch (error) {
